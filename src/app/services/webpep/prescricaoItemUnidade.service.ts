...
@Injectable()
export class PrescricaoItemUnidadeService {
    url: string = 'web/pep/';
    private servidor;
    ...
    get(objParams) {
        const url = `${this.url}prescricaoItemUnidade/${Sessao.getToken()}`;
    ...
    atualizar(id, objTipo) {
        const url = `${this.url}prescricaoItemUnidade/${Sessao.getToken()}/${id}`;

...
@Injectable()
export class PrescricaoItemEspecialidadeService {
    url: string = 'web/pep/';
    private servidor;
    ...
    get(objParams) {
        const url = `${this.url}prescricaoItemEspecialidade/${Sessao.getToken()}`;
    ...
    atualizar(id, objTipo) {
        const url = `${this.url}prescricaoItemEspecialidade/${Sessao.getToken()}/${id}`;

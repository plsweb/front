...
@Injectable()
export class PacientePrescricaoProdutoService {
    url: string = 'web/pep/';
    private servidor;
    ...
    get(objParams) {
        const url = `${this.url}pacientePrescricaoProduto/${Sessao.getToken()}`;
    ...
    atualizar(id, objTipo) {
        const url = `${this.url}pacientePrescricaoProduto/${Sessao.getToken()}/${id}`;
